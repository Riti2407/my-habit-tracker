...
function App() {
  ...
  const handleCompletion = (habitKey, dateString) => {
    setCompleted((prev) => {
      ...
      const dayData = habitData[dateString] || { completed: false, note: "" };

      const updated = {
        ...prev,
        [habitKey]: {
          ...habitData,
          [dateString]: { ...dayData, completed: !dayData.completed },
        },
      };
  ...
  // Save a note for a habit
  const handleNoteSave = (habitKey, dateString, note) => {
    setCompleted((prev) => {
      const habitData = prev[habitKey] || {};
      const dayData = habitData[dateString] || { completed: false, note: "" };
  ...
  const totalCompleted = Object.values(completed).reduce((sum, days) => {
    const completedCount = Object.values(days).filter(day => day && day.completed).length;
